...
Script Version: V0.2 - Initial version with basic data insertion for testing and development purposes.
...
DECLARE
    ...
    entropy_seed BYTEA := NULL;
    -- Final seed value
    final_seed NUMERIC;
BEGIN
    -- Check if we can get entropy from pg_read_file
    -- This is not guaranteed to work, depends on server configuration
    IF has_function_privilege('pg_read_file(bytea,text,bigint)', 'execute') THEN
        entropy_seed := pg_read_file('/dev/urandom', 8); -- or another source of entropy
    ELSE
        -- Fallback if pg_read_file isn't available
        -- We use a simple hash of session and machine info as entropy
        entropy_seed := substring(CAST(MD5(session_seed || machine_seed) AS BYTEA), 1, 8);
    END IF;
...
INSERT INTO sensor (sensor_id, sensor_type_id, zone_id, system_id, location, installation_date)
SELECT 
...
SELECT 
    ...
    (SELECT sensor_id FROM sensor ORDER BY random() LIMIT 1),  -- Randomly selects a sensor ID.
...
INSERT INTO systemwide_alert (sensor_id, zone_id, system_id, alert_type, alert_message)
SELECT 
